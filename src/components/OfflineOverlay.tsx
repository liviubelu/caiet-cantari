...
function useOnlineStatus() {
  const [isOnline, setIsOnline] = useState(true) // optimistic for SSR
...
export function OfflineOverlay() {
  ...
  useEffect(() => {
    if (!isOnline) {
      getAllSongs().then((s) => {
        ...
        // If the URL was /song/[id], auto-open that song
      ...
      // Back online — hide overlay and reset state
      setReady(false)
      setSelected(null)
      setQuery("")
    }
  ...
  // Offline but IndexedDB not loaded yet
  if (!ready) {
    return (
      <div className="fixed inset-0 z-50 bg-white flex items-center justify-center">
        <p className="text-sm text-gray-400">Se încarcă melodiile…</p>
      </div>
    )
  }
